...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyLayout(mode) {
    ...
    [btnPile, btnScatter, btnGrid, btnFan].forEach((btn) =>
  ...
  function setupCardInteractivity(card) {
    ...
    function onPointerDown(e) {
      if (e.target.closest(".action-btn") || e.target.closest(".flip-back-btn"))
        return;
    ...
    function onPointerMove(e) {
      ...
      const rot =
        parseFloat(card.dataset.currentRot) ||
        parseFloat(card.dataset.rot) ||
        0;
  ...
  function showCinematicHint() {
    ...
    const topCard = cards.reduce((prev, current) => {
      return (parseInt(current.style.zIndex) || 0) >
        (parseInt(prev.style.zIndex) || 0)
        ? current
        : prev;
    });
    ...
    if (activePhotoIdForModal !== null) {
      ...
      if (photo && modalNoteInput) {
        ...
        const card = document.querySelector(
          `.photo-card[data-id="${photo.id}"] .note-text`,
        );
  ...
  function navigateModal(direction) {
    ...
    currentModalIndex =
      (currentModalIndex + direction + photos.length) % photos.length;
    updateModalContent();
  }
  ...
  // Attempt playing cover media as soon as the page and audio are ready.
  ...
  // Fallback for browsers that still block autoplay until the user interacts.
  const unlockAudioOnFirstInteraction = () => {
    if (isCoverActive) {
      startCoverMedia();
    }
  };
  ["touchstart", "touchend", "pointerdown", "click", "scroll"].forEach(
    (evt) => {
      window.addEventListener(evt, unlockAudioOnFirstInteraction, {
        once: true,
      });
    },
  );
  ...
  if (coverNextBtn && coverOverlay) {
    coverNextBtn.addEventListener("click", () => {
      ...
      // Explicitly stop the opening MHA track before switching to the birthday album
      stopCoverMusic();
      startAlbumMusic();
